...
-- ModelbuilderT is actually a type that describes a transformations... later on anything that describes a transformation
-- can be considered a value of this type.
-- as a Monad: (ModelBuilderT m) is the m part of a Monad

newtype ModelBuilderT m a = ModelBuilderT (S.StateT BasicPDEModel m a)
type ModelBuilder a = ModelBuilderT I.Identity a            


--unwraps the ModelBuilderT from a
...
--allows functionality of the normal get and put functions with the Monad transformer, now it is in the ModebuilerT      
-- Monad it can be functionally used
...
-- Anything with an X attacthed to the end means that is is take out of a Monad, thus we can
...
----(?????)----- these are for building Monadic Boolean expressions
...
--modified so the initial value is always with respect to the variable for time
      

initialValueM bv v iv =
  newBoundaryEq (realConstant bv .==. time) (return v) (realConstant iv)
...
tryEvaluateRealAsConstant (XSpaceE) = Nothing
tryEvaluateRealAsConstant (YSpaceE) = Nothing
tryEvaluateRealAsConstant (ZSpaceE) = Nothing
tryEvaluateRealAsConstant (TimeE) = Nothing
